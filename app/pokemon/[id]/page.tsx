    ...
    <main className="min-h-screen flex">
      <div className="flex flex-col gap-8 p-16 w-2/3 bg-white">
        <div>
          ...
          <div className="flex gap-2 mt-3">
            {pokemon.types.map(({ type }) => (
              <span 
                key={type.name} 
                className={`${typeColors[type.name]?.bg} text-white text-sm px-6 py-1 rounded-full capitalize`}
              >
                {type.name}
              </span>
            ))}
        ...
        <div className="flex gap-8">
          ...
          <div>
            ...
            <p className="text-zinc-800 font-bold capitalize">
      ...
      <div className={`w-1/3 ${color.bg} flex items-center justify-center`}>
        <img 
          src={sprite} 
          alt={pokemon.name} 
          className="object-contain drop-shadow-2xl w-80 h-80" 
        />
      </div>
    </main>
  );
