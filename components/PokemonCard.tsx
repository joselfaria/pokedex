...
export default function PokemonCard({ pokemon }: { pokemon: { id: number; name: string; types: string[] } }) {
  ...
  return (
    <Link href={`/pokemon/${pokemon.id}`}>
      <div className={`border rounded-2xl p-4 flex flex-col bg-gradient-to-b ${typeColors[pokemon.types[0]].gradient} to-gray-300 items-center text-black gap-2 hover:scale-105 transition-transform cursor-pointer`}>
        ...
        <div className="flex gap-1">
          {pokemon.types.map((type) => (
              <span key={type} className={` ${typeColors[type].bg} text-white text-sm px-4 py-1 rounded-full capitalize`}>
              {type}
            </span>
          ))}
        </div>
